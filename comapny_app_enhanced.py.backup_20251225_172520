...
def candidate_detail(user_id):
    ...
    try:
        ...
        # 検索履歴を取得（user_interactionsから）
        cur.execute("""
            SELECT 
                metadata->>'search_keywords' as search_keywords,
                job_id,
        ...
        for history in search_history:
            search_keywords = history.get('search_keywords')
            if search_keywords:
                # JSONBから取得した文字列を処理
                if isinstance(search_keywords, str):
                    # JSON配列形式 ["Python", "リモート"] の場合
                    if search_keywords.startswith('['):
                        import json
                        keywords = json.loads(search_keywords)
                    # カンマ区切り形式 "Python,リモート" の場合
                    else:
